...
import { Award, Lock, Check, Star, Flame } from 'lucide-react'
...
export function getAchievementsList(language: string) {
  if (language === 'tr') {
    return [
      ...
      { id: 'kadikoy_bogazi', name: 'Kadıköy Boğazı', desc: 'Fenerbahçe galibiyetlerinde 3 kez tam skor tahmini yapın.', req: '3 kez FB galibiyeti tam skoru', rarity: 'common' as const },
    ...
    return [
      ...
      { id: 'kadikoy_bogazi', name: 'Kadıköy Boğazı', desc: 'Achieve 3 exact tips on Fenerbahçe wins.', req: '3 exact FB wins correctly predicted', rarity: 'common' as const },
  ...
  return [
    ...
    { id: 'kadikoy_bogazi', name: 'Kadıköy Boğazı', desc: 'Erziele 3 exakte Tipps auf Siege von Fenerbahçe.', req: '3 exakte FB-Siege richtig getippt', rarity: 'common' as const },
...
export function AchievementsSection({ unlockedSet, newlyUnlocked }: AchievementsSectionProps) {
  ...
  const sortedAchievements = useMemo(() => {
    return [...achievementsList].sort((a, b) => {
      ...
      const aRarity = mapRarity(a.rarity)
      const bRarity = mapRarity(b.rarity)
      const aOrder = RARITY_CONFIG[aRarity]?.order ?? 99
      const bOrder = RARITY_CONFIG[bRarity]?.order ?? 99
