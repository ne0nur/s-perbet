...
const FRAGEN = [
  { id: 1, text: 'Wer wird Meister?' },
  { id: 2, text: 'Welches Team schießt die meisten Tore?' },
  { id: 3, text: 'Welches Team kassiert die wenigsten Gegentore?' },
]
  ...
  return (
    <div className={`bg-surface-container-low border rounded-xl p-4 shadow-sm stagger-in transition-all duration-300 ${
      ...
      <div className="flex items-center gap-2 mb-4 pb-3 border-b border-surface-container-high/60">
        ...
        <h3 className="text-xs font-mono text-on-surface uppercase tracking-wider font-bold">Meine Bonus-Tipps</h3>
      </div>

      <div className="space-y-3 animate-fade-in text-left">
          {bonusGesperrt && (
            <div className={`glass-panel p-2 flex items-center gap-2 ${!TIPPS_FREIGESCHALTET ? 'bg-amber-500/5 border-amber-500/10' : 'border-amber-500/20'}`}>
              <Lock size={12} className="text-amber-400 shrink-0" />
              <p className="text-[10px] text-amber-500 leading-tight">
                {!TIPPS_FREIGESCHALTET
                  ? 'Bonus-Tipps ab Spielplan-Veröffentlichung verfügbar.'
                  : 'Bonus-Tipps sind nach dem 2. Spieltag gesperrt.'}
              </p>
            </div>
          )}
                  ...
                  {bonusGesperrt ? (
                    <div className="flex items-center gap-3 bg-black/20 border border-white/5 rounded-lg py-2.5 px-3">
                      ...
                      <div className="flex flex-col">
                        <span className="text-on-surface text-sm font-bold">{wert || 'Kein Tipp abgegeben'}</span>
                        {gespeicherterTipp && (
          ...
          {TIPPS_FREIGESCHALTET && !bonusGesperrt && (
            <button
            ...
            >
              {gespeichert ? <span className="flex items-center justify-center gap-2"><Check size={14} /> Gespeichert</span> : 'Bonus-Tipps abgeben / aktualisieren'}
            </button>
          )}
      </div>
    </div>
